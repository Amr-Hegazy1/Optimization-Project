...
To rigorously evaluate the optimization performance and consistency of Simulated Annealing (SA), Genetic Algorithm (GA), and Ant Colony Optimization (ACO), we conducted a comprehensive benchmarking study with 20 independent runs of each algorithm. This section presents the comparative analysis of key performance metrics.
...
Table~\ref{tab:performance_indices_aco} presents the comparative performance metrics for all three algorithms over 20 independent runs.
...
\hline
\textbf{Metric} & \textbf{Simulated Annealing} & \textbf{Genetic Algorithm} & \textbf{Ant Colony Optimization} \\
\hline
Optimal Solution & $(13, 8), (5, 7), \ldots$ & $(6, 2), (9, 0), \ldots$ & $(17, 19), (8, 13)\ldots$ \\
\hline
Optimal Fitness Value & $13.7049$ & $12.3180$ & $11.4888$ \\
\hline
Mean Fitness & $14.3852$ & $14.3379$ & $11.7522$ \\
\hline
Standard Deviation & $3.6271 \times 10^{-1}$ & $2.1899 \times 10^{0}$ & $1.1334 \times 10^{-1}$ \\
\hline
Computational Time & $34.867$ sec & $165.515$ sec & $130.991$ sec \\
\hline
\end{tabular}
\end{table}
...
The benchmark results reveal significant differences in the optimization characteristics of the three algorithms:

\begin{itemize}
    \item \textbf{Solution Quality:} 
    Ant Colony Optimization achieved the best optimal fitness value of $11.4888$, outperforming Genetic Algorithm ($12.3180$) and Simulated Annealing ($13.7049$). ACO's ability to construct solutions using both pheromone trails and heuristic information allows it to find superior paths that balance coverage and connectivity more effectively than the other methods.

    \item \textbf{Consistency and Reliability:}
    ACO demonstrated the highest consistency with the lowest standard deviation ($0.1133$), compared to SA ($0.3627$) and GA ($2.1899$). This indicates that ACO reliably converges to high-quality solutions across different runs, whereas GA exhibits significant variance, likely due to the stochastic nature of its crossover and mutation operators and the random initial population.

    \item \textbf{Computational Efficiency:}
    Simulated Annealing remains the fastest algorithm ($34.867$ sec), but its solution quality is the lowest. ACO ($130.991$ sec) is faster than GA ($165.515$ sec) while delivering significantly better results. This suggests that ACO offers a better trade-off between solution quality and computational time for this specific multi-robot path planning problem.
\end{itemize}
