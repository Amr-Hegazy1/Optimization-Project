...
We introduce \textit{\systemname{}}, an adaptive path planning framework that utilizes a machine learning router to select the most appropriate metaheuristic for a given scenario. By characterizing the problem instance through topological features, our system intelligently switches between SA, GA, and ACO.
...
We model the environment as a discrete grid map $M \in \{0, 1\}^{H \times W}$, where 0 represents free space and 1 represents obstacles. A team of $R$ robots must plan paths $P = \{p_1, \dots, p_R\}$ over a time horizon $T$. The objective function $J(P)$ minimizes a weighted cost:

\begin{equation}
    J(P) = \frac{\alpha}{|C(P)| + \epsilon} + \frac{\beta}{\sum_{t} \sum_{i<j} w_{ij}(t) + \epsilon} + \gamma \cdot \Omega(P)
\end{equation}

where $C(P)$ is the set of unique cells covered, $w_{ij}(t)$ is the communication link quality between robot $i$ and $j$ at time $t$ (inversely proportional to distance), and $\Omega(P)$ is a penalty term for network disconnections. The weights $\alpha, \beta, \gamma$ allow mission operators to prioritize exploration vs. safety.
...
\begin{tikzpicture}[
  ...
  \tikzstyle{router} = [rectangle, rounded corners=3pt, minimum width=3cm, minimum height=1.2cm, draw=primary, fill=primary!10, line width=1pt]
   
  % Input
  \node[block, fill=lightgray] (input) {Input Map \& Config};
  \node[block, below=of input] (features) {Feature Extraction};
  \node[router, below=of features] (router) {\textbf{ML Router} \\ (Random Forest)};
...
For learning-based routing, we evaluate on the real router dataset collected by running SA, GA, and ACO on the same scenario and labeling each instance by the algorithm achieving the minimum objective cost (ties broken by runtime). We use the combined dataset with $N=9{,}179$ instances.
...
\midrule
Static (SA) & 11.7\% & 95.8\% & 0.96 \\
Static (GA) & 33.8\% & 93.0\% & 0.93 \\
Static (ACO) & \textbf{54.5\%} & 96.0\% & 0.96 \\
Static (ABC) & 0.5\% & 94.0\% & 94\% \\
\textbf{\systemname{}} & \textbf{94.99\% (Acc)} & \textbf{0.95 (Avg)} & \textbf{0.95} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Variant} & \textbf{Acc.} & \textbf{$\mathbb{P}(\hat{y}=\text{SA}\mid y=\text{ACO})$} & \textbf{Mean Rel. Regret} & \textbf{$t_{\text{SA}}$} \\
\midrule
...
\begin{tikzpicture}
  \begin{axis}[
    xbar,
    width=0.5\textwidth,
    height=5.5cm,  % Increased height for better spacing
    symbolic y coords={Conn. Threshold,Start Edge Frac,Min Pairwise Euclid,Energy Budget,Min Pairwise Manh,Path Length},
    ytick=data,
    ...
    nodes near coords style={font=\footnotesize, inner sep=-4pt, xshift=11pt},
    point meta=explicit symbolic,  % Use explicit values for labels
    yticklabel style={font=\small, align=right},  % Better alignment
    y tick label style={text width=4.5cm, align=right },  % Fixed width for labels
    bar width=14pt,  % Slightly larger bars
    grid=major,
    grid style={line width=0.3pt, gray!30},  % Lighter grid
    axis line style={-},
    tick style={major tick length=0pt},  % Remove tick marks
    enlarge y limits=0.15  % Add spacing between bars
  ]
    \addplot[
      fill=primary,
      draw=blue!40!black,
...
The results demonstrate a clear "No Free Lunch" dynamic:
\begin{itemize}
    \item \textbf{ACO} is the heavy lifter, essential for tight connectivity constraints and sparse initial communication graphs ($\approx 54.5\%$ of router instances).
    \item \textbf{SA} is the specialist for easier instances with dense initial graphs ($\approx 11.8\%$ of cases), offering orders-of-magnitude speedup.
    \item \textbf{GA} serves as the generalist middle-ground.
\end{itemize}

One limitation is the grid scale ($20 \times 20$). However, the combinatorial complexity ($4^{R \times T}$) remains vast, and the topological features driving the router (e.g., edge density) are scale-invariant. Thus, we expect the routing logic to generalize to larger maps.
